...
void FbxLoader::getBoneOffsets(FbxScene *scene, FbxNode *node, std::vector<XMFLOAT4X4> &boneOffsets, const std::vector<std::string> &boneNames) {
  ...
  for (int deformerIndex = 0; deformerIndex < mesh->GetDeformerCount(); ++deformerIndex) {
    ...
    for (int clusterIndex = 0; clusterIndex < skin->GetClusterCount(); ++clusterIndex) {
      ...
      for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
          boneOffset.m[i][j] = static_cast<float>(globalBindposeInverseMatrix.Get(i, j));
        }
      }
  ...
  for (int deformerIndex = 0; deformerIndex < mesh->GetDeformerCount(); deformerIndex++) {
    ...
    for (int clusterIndex = 0; clusterIndex < skin->GetClusterCount(); clusterIndex++) {
      ...
      for (int i = 0; i < cluster->GetControlPointIndicesCount(); i++) {
        ...
        boneIndexWeight.boneWeight = static_cast<float>(cluster->GetControlPointWeights()[i]);
...
void FbxLoader::getVertexInfoDynamic(FbxNode *node, std::vector<VertexDynamic> &vertices,
...
void FbxLoader::getVertexInfoStatic(FbxNode *node, std::vector<VertexStatic> &vertices,
  std::unordered_map<int, ControlPoint> &controlPoints) {
  FbxMesh *mesh = reinterpret_cast<FbxMesh *>(node->GetNodeAttribute());
...
Model FbxLoader::loadFBXModel(ID3D11Device *device, const std::string &name) {
  ...
  // Get bone offsets, control points, vertex info
  {
    for (int i = 0; i < root->GetChildCount(); ++i) {
      ...
      if (type == FbxNodeAttribute::eMesh) {
        ...
        getVertexInfoDynamic(child, vertices, controlPoints);
      }
    }
  }
...
Animation *FbxLoader::loadFBXModelAnimation(ID3D11Device *device, const std::string &name) {
  FbxImporter *importer = FbxImporter::Create(manager, "");
  FbxScene *scene = FbxScene::Create(manager, "");
...
MeshStatic *FbxLoader::loadFBXModelMesh(ID3D11Device *device, const std::string &name) {
  FbxImporter *importer = FbxImporter::Create(manager, "");
  FbxScene *scene = FbxScene::Create(manager, "");
  ...
  // Get bone offsets, control points, vertex info
  {
    for (int i = 0; i < root->GetChildCount(); ++i) {
      ...
      if (type == FbxNodeAttribute::eMesh) {
        ...
        getVertexInfoStatic(child, vertices, controlPoints);
      }
    }
  }
