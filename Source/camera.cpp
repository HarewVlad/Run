...
void Camera::attachCamera(XMVECTOR position, XMVECTOR forward, XMVECTOR right, XMVECTOR up) {
  updateFollowTarget(position, forward, right, up);
...
void Camera::updateFollowTarget(XMVECTOR position, XMVECTOR forward, XMVECTOR right, XMVECTOR up) {
  const int forwardScalingFactor = 80;
  const int upScalingFactor = 30;

  this->position = position - forward * forwardScalingFactor + up * upScalingFactor;
