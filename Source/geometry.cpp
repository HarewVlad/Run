...
MeshStatic *GeometryGenerator::createPlaneFromHeightmap(ID3D11Device *device, const std::string &filename) {
...
MeshStatic *GeometryGenerator::createPlaneDefault(ID3D11Device *device, XMFLOAT3 pos, float width, float depth, UINT m, UINT n) {
  UINT vertexCount = m * n;
  UINT faceCount = (m - 1)*(n - 1) * 2;

  float halfWidth = 0.5f*width - pos.x;
  float halfDepth = 0.5f*depth + pos.z;
  float dx = width / (n - 1);
  float dz = depth / (m - 1);
  float du = 1.0f / (n - 1);
  float dv = 1.0f / (m - 1);

  std::vector<VertexStatic> vertices(vertexCount);
  for (UINT i = 0; i < m; ++i)
  {
    ...
    for (UINT j = 0; j < n; ++j)
    {
      ...
      vertices[i*n + j].pos = { x, pos.y, z };
      vertices[i*n + j].normal = { 0, 1, 0 };
      vertices[i*n + j].tex = { j * du, i * dv };
    }  }

  std::vector<int> indices(faceCount * 3);
  UINT k = 0;
  ...
  ID3D11Buffer *vertexBuffer = nullptr;
  {
    ...
    vertexBufferDesc.ByteWidth = sizeof(VertexStatic) * vertexCount;
...
MeshStatic *GeometryGenerator::createSphereDefault(ID3D11Device *device, int latLines, int longLines) {
...
MeshStatic *GeometryGenerator::createBoxDefault(ID3D11Device *device, int w, int h, int d) {
